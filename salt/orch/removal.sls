...
{%- if target in masters %}
  {# we must highstate: *}
  {# * minions (ie, haproxy in all the minions) #}
  {%- do affected_roles.append('kube-minion') %}
{%- endif %}

{%- if target in minions %}
  {# ok, /etc/hosts will contain a wrong entry, but who cares #}
{%- endif %}
...
{%- if affected_roles %}
  ...
  {%- if replacement %}
    ...
    {%- set affected_expr = affected_expr + ' and not L@' + replacement %}
  {%- endif %}

  {%- do salt.caasp_log.log('CaaS: applying high state in affected machines: %s', affected_expr) %}

highstate-affected-{{ affected_roles|join('-') }}:
  salt.state:
    - tgt: {{ affected_expr }}
    - tgt_type: compound
    - highstate: True
